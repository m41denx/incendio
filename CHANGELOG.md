...
## What's Changed
### ✨ New Features
- (feat) Kubernetes: **Clusters** is now the main page — a list of agent-managed clusters (name, status, Kubernetes version, control-plane/worker counts, project) with SWR-style polling and per-cluster delete, plus **Create cluster** and **Kubernetes settings** actions. Creating a cluster is a dedicated multi-section form (Cluster, Machines, Infrastructure credentials, Generated artifacts) with side-navigation, matching the rest of Incendio's create/edit panels
...
- (feat) Kubernetes: **agent integration** lives in a dedicated **Kubernetes settings** view — configure the self-hosted `@incendio/k8s` agent (URL + bearer token, stored per-browser) and **Test connection** to handshake `GET /v1/info` (version + capabilities + TLS fingerprint). A **Management appliance** tab provisions the CAPI/CAPN control plane as a privileged, nested Incus **container** running single-node k3s, in its own `incendio-mgmt` project sized on its own defaults and exposed via a host proxy device on `:8843`; you approve the appliance's self-signed certificate once. Creating a cluster then stamps a per-cluster Incus project via the agent. Without an agent, the pure-browser artifact generator still works.
