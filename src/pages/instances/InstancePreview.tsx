...
const InstancePreview: FC<Props> = ({ instance, refetch, onFailure }) => {
  const queryOptions = refetch
    ? {
        ...
        queryFn: async () => fetchInstancePreview(instance),
  ...
  const { data: imgData, error, isLoading } = useQuery(queryOptions);

  if (error) {
    onFailure("Loading instance preview failed", error);
  }
